...
function mul(A::MatA,B::MatA,atol::Float64)::MatA
    ...
    C = MatA(A.size,Matrix{TensorDec}(undef,3,3))
...
function mul(A::MatA,B::MatC,atol::Float64)::MatC
    ...
    C = MatC(A.size,Vector{TensorDec}(undef,3))
...
function mul(A::MatB,B::MatA,atol::Float64)::MatB
    ...
    C = MatB(A.size,Vector{TensorDec}(undef,3))
...
function mul(A::MatB,B::MatC,atol::Float64)::TensorDec
    if A.size != B.size
        throw("sizes of A and B not compatible")
    end
...
function get_NSFMatCoefs(X::TensorDec,m::Int,H::Vector{Float64},G::Vector{Float64},atol::Float64)::NSFMatCoefs
    ...
    mat = Matrix{TensorDec}(undef,4,4)
    for i in 1:2, j in 1:2, k in 1:2, l in 1:2
        mat[2*(k-1)+l,2*(i-1)+j] = TensorDec(X.size÷2,X.rank,X.rank,copy(X.sig),
    ...
    A = MatA(X.size÷2,Matrix{TensorDec}(undef,3,3))
    for i in 1:3, j in 1:3
...
#     atol2 = atol/9
#     # atol2 = atol
#     T = MatA(A.size,Matrix{TensorDec}(undef,3,3))
#     for i in 1:3, j in 1:3
#         if i == j
...
"""
function invMatA(A::MatA,atol::Float64;max_iter=50)::Union{Nothing,MatA}
    # Select α
    ...
    for _ in 1:max_iter
        ...
        B = MatA(A.size,Matrix{TensorDec}(undef,3,3))
        for i in 1:3
        ...
        # Compute off-diagonal blocks of 2G0-G0*A*G0
        # Use max svalue of the diagonal blocks for truncation
        maxsval = max(get_max_sval(B.values[1,1]),
            get_max_sval(B.values[2,2]),
            get_max_sval(B.values[3,3]))
        println("invMatA: maximim svalue is $(maxsval)")
        ...
        for i in 1:3
            for j in 1:3
                if i != j
                    ...
                    reduce(B.values[i,j],atol2,maxsig=maxsval)
                end
            end
        end
...
function invMatAr(A::MatA,atol::Float64,rank::Matrix{Int64};max_iter=50)::Union{Nothing,MatA}
    if size(rank) != (3,3)
...
"""
function invMatAr2(A::MatA,atol::Float64;max_iter=50)::Union{Nothing,MatA}
    # Select α
    # atol2 = min(1e-3,atol*1e2)
    atol2 = 1e-4
    α = 1/snorm(A,atol2)
...
function factorize(X::NSFMatCoefs,atol::Float64)::NSFMatCoefs
    ...
    maxsval = max(get_max_sval(Y.A.values[1,1]),
        get_max_sval(Y.A.values[2,2]),
        get_max_sval(Y.A.values[2,2]))
    println("factorize: max svalue is $(maxsval)")

    Y.B = copy(X.B)
    for j in 1:3
        reduce(Y.B.values[j],atol2,maxsig=maxsval)
    end
    ...
    for j in 1:3
        reduce(Y.C.values[j],atol2,maxsig=maxsval)
    end

    Y.T = copy(X.T)
    # atol2 = atol/16
    atol2 = atol
    axpy(-1.,mul(X.B,Y.C,atol2),Y.T,atol2/1e5)
    reduce(Y.T,atol2,maxsig=maxsval)
...
function get_NSFMatrix(X::TensorDec,
        moments::Int,
        scales::Int,
        atol::Float64,
        )::NSFMatrix
...
# function get_NSFMatrix_decomp(X::TensorDec,
#         moments::Int,
#         scales::Int,
#         atol::Float64,
#         )::NSFMatrix
