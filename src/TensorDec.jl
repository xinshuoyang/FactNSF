...
"""
Make a copy of TensorDec
"""
copy(A::TensorDec) = TensorDec(A.size,A.capacity,A.rank,copy(A.sig),copy(A.u),copy(A.v))

"""
Populate s-values in descending order
"""
function get_svals(A::TensorDec)::Vector{Float64}
...
"""
function get_max_sval(A::TensorDec)::Float64
    if A.rank == 0
        throw("attempt to retrive max s-value of an empty TensorDec")
    end
...
"""
Create a zero TensorDec of the same size as A and with a given (optional) capacity
"""
create_zeros(size::Int,capacity::Int=0) = TensorDec(size,capacity,0,
    zeros(Float64,capacity),[spzeros(size) for i in 1:capacity],[spzeros(size) for i in 1:capacity])

"""
Create a identity TensorDec of the same size as A and with a given (optoinal) capacity
"""
function create_identity(size::Int,capacity::Int=1)
    ...
    return TensorDec(size,1,capacity,sig,u,v)
end

"""
Increase the capacity of TensorDec
"""
function resize(A::TensorDec,capacity::Int)::Nothing
...
"""
function diagvec(A::TensorDec)::Vector{Float64}
...
"""
function get_diag(A::TensorDec)::TensorDec
    ...
    return TensorDec(A.size,A.rank,A.rank,sig,u,v)
end

"""
Get a copy of the off-diagonals of A
"""
# function get_offdiag(A::TensorDec,atol::Float64=1e-15)::TensorDec
#     max_rank = 2*A.rank
...
#     B = TensorDec(A.size,rank,rank,sig[1:rank],u[1:rank],v[1:rank])
...
Normalize TensorDec
"""
function normalize(A::TensorDec,atol::Float64)::Nothing
    atol2 = atol*atol
    rank = 0
    for i in 1:A.rank
...
"""
Convert TensorDec to dense matrix
"""
function to_dense(A::TensorDec)::Matrix{Float64}
...
"""
Plus a constant times another TensorDec
Y <- alpha * X + Y
...
X <- alpha * I + X
"""
function aipy(alpha::Float64,X::TensorDec)::Nothing
...
"""
Perform multiplication between two TensorDec A and B
C <- A * B
"""
function mul(A::TensorDec,
    B::TensorDec,
    atol::Float64;
    )::TensorDec
...
"""
function diagmul(A::TensorDec,B::TensorDec)::Vector{Float64}
    size = A.size
    if size != B.size
        DimensionMismatch("first TensorDec has size $(A.size) which does not match the size of the second, $(B.size)")
    end
...
"""
Scale TensorDec by constant
"""
function scale(A::TensorDec,alpha::Float64)::Nothing
...
"""
Compute inverse of a diagonal TensorDec
"""
# function get_inv_diag(A::TensorDec,atol::Float64;max_iter=20)::TensorDec
...
"""
Apply (multiply) TensorDec to dense matrix
"""
function apply(A::TensorDec,X::Matrix{Float64};TRANS::String="N")::Matrix{Float64}
    # Check inputs
...
"""
Reduce the rank of TensorDec via Gram-Schmidt Orthogonalization
...
"""
function reduce(A::TensorDec,atol::Float64;maxsig::Float64=0.,maxiter::Int=1)::Nothing
    ...
    for _ in 1:maxiter
        ...
        for j in 1:rank
            ...
            if maxsig > 0.
                ...
                if A.sig[mm]/A.sig[ipivot[1]] <= atol2
                    rank = j-1
                    break
                end
            end
            ...
            for l in j+1:rank
                ...
                if maxsig > 0.
                    ...
                    if A.sig[ll]/A.sig[ipivot[1]] <= atol2
                        A.sig[ll] = 0.
                        @goto next_uvec
                    end
                end
...
# function reduce(A::TensorDec,atol::Float64)::Nothing
...
# function reduce_abs(A::TensorDec,atol::Float64;maxiter::Int=10)::Nothing
...
# """
# Reduce the number of terms in TensorDec using Cholesky decomposition.
# """
# function fastchol(A::TensorDec;rankmax::Union{Int,Nothing}=nothing,atol::Float64=1e-7)::Nothing
...
"""
function droprank(A::TensorDec,rank::Int64)::Nothing
...
function droptol(A::TensorDec,atol::Float64)::Nothing
...
function invsqrt(A::TensorDec,atol::Float64;max_iter::Int=200)::TensorDec
    """
    Compute inverse square root of tensor product
         n
    A = sum u_j ⊗ v_j
        j=1
